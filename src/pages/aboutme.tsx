...
export default function About({photos}: { photos: TPhoto[] }) {
    ...
    const getPhoto = useCallback((images: TPhoto[], index: number) => (images?.[index] ?
            <PhotoCard img={images[index]} classes={styles.main__image}/> : null),
        []
    )
    return (
...
export const getStaticProps: GetStaticProps<{ photos: TPhoto[] }> = async (context) => {
    try {
        const albumsRes = axios.get(`https://api.vk.com/method/photos.getAlbums?owner_id=${process.env.NEXT_PUBLIC_MY_ID}&access_token=${process.env.NEXT_PUBLIC_ACCESS_TOKEN}&need_system=1&v=${process.env.NEXT_PUBLIC_VK_VERSION}`).then(res => res.data);
        const albums: TAlbum[] = await albumsRes.then(res => res.response.items);
        const myPagePhotosID = albums.find(e => e.title.includes("profile photos"))?.id;
        const myPagePhotosRes = axios.get(`https://api.vk.com/method/photos.get?owner_id=${process.env.NEXT_PUBLIC_MY_ID}&access_token=${process.env.NEXT_PUBLIC_ACCESS_TOKEN}&extended=1&v=${process.env.NEXT_PUBLIC_VK_VERSION}&album_id=${myPagePhotosID}`).then(res => res.data);
        const photos: TPhoto[] = await myPagePhotosRes.then(res => res.response.items);
        return {
            props: {
